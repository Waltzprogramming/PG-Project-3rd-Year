...
namespace {
    ...
    constexpr float VanModelDisplaySize = 2.80f;
...
struct Mapa1::Impl {
    ...
    bool loadCornProjectileModel() {
        projectileCornModel = {};
        projectileCornModel.model = ModelLoader::loadModel(resolveAssetPath("assets/mapa1/extra/maiz_low_poly/scene.gltf"));
    ...
    void drawVanShop(float now) {
        ...
        if (ensureVanModelLoaded()) {
            ...
            const float maximumExtent = std::max({ extents.x, extents.y, extents.z, 0.001f });
            const float vanScale = 0.001f;
            const float scaledHeight = 0.1f * vanScale;
